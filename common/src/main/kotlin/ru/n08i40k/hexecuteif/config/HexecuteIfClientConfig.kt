...
object HexecuteIfClientConfig {
    ...
    fun init() {
        ...
        holder.registerSaveListener { _, config ->
            HexecuteIfServerConfig.holder.config = HexecuteIfServerConfig.GlobalConfig(config.server)
            InteractionResult.PASS
        }

        // when we leave a server, clear our local copy of its config
        ClientPlayerEvent.CLIENT_PLAYER_QUIT.register { _ ->
            HexecuteIfServerConfig.onSyncConfig(null)
        }
    }
    ...
    class GlobalConfig : GlobalData() {
        ...
        // this should only be used inside of this class; use HexecuteIfServerConfig.config to access the server-side config in other code
        @Category("server")
        @TransitiveObject
        val server = HexecuteIfServerConfig.ServerConfig()
    }
